...
export default function StudentPaymentInvoice() {
  ...
  useEffect(() => {
    ...
    const fetchInvoiceData = async () => {
      ...
      // URL එකෙන් parameters ලබාගැනීම (?s=STUDENT_ID&m=YYYY-MM)

      const urlParams = new URLSearchParams(window.location.search);

      const studentId = urlParams.get('s');

      const monthKey = urlParams.get('m'); // උදා: 2026-03
      ...
      if (studentData && configData?.class_rates_text) {
        ...
        // සිසුවා සම්බන්ධ වී ඇති පන්ති (class_types) වල මිල ගණන් පමණක් එකතු කිරීම

        const activeClasses: string[] = studentData.class_types || [];
    ...
    return (
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        ...
        <div className="bg-slate-900 border border-red-500/20 p-6 rounded-2xl max-w-sm text-center">
          ...
          <p className="text-slate-400 text-xs mt-2">මෙම බิල්පත විවෘත කිරීමට ඔබට අවසර නැත. කරුණාකර නිවැරදි ලින්ක් එක භාවිතා කරන්න.</p>
