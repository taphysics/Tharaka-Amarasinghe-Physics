...
const LiveClassPlayer = ({ currentUser }: { currentUser: Student | null }) => {
  ...
  const checkStudentAccess = async (liveClass: ScheduledLive) => {
    ...
    try {
      ...
      if (liveClass.target_month) {
        const parts = liveClass.target_month.split('-');
        if (parts.length === 2) {
          targetYear = parts[0];
          targetMonth = parts[1];
        } else {
          targetMonth = liveClass.target_month;
        }
      }

      const recMonthStr = String(targetMonth).trim();
      const recYearMonthStr = `${targetYear}-${targetMonth}`;
      const standardizedDbMonth = formatYearMonth(targetYear, targetMonth);

      // 1. Free plan / Free Month පරීක්ෂාව
      const isGloballyFree = currentUser.plan_type?.toLowerCase() === 'free';
      const isThisMonthFree = currentUser.free_months?.includes(recMonthStr) || 
                              currentUser.free_months?.includes(recYearMonthStr) || 
                              currentUser.free_months?.includes(standardizedDbMonth);

      if (isGloballyFree || isThisMonthFree) {
        setHasAccess(true);
        return;
      }

      // 2. Payments Table එකෙන් ගෙවීම් පරීක්ෂාව
      ...
      const paymentRecord = payData?.find((p: any) => {
        ...
        const rClass = String(liveClass.class_type || "").toLowerCase().trim();
      ...
      console.error("Error validating payment access:", err);
      setHasAccess(false);
    }
  };
