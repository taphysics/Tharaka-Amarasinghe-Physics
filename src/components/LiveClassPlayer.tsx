...
const LiveClassPlayer = ({ currentUser }: { currentUser: Student | null }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Class Filter කරන Function එක (StudentRecordings හි ඇති ආකාරයටම ලිහිල් කර ඇත)
      const isMatch = (type1?: string, type2?: string, arr?: string[]) => {
        if (!hasClasses) return false;
        
        const sClasses = studentClasses.map(c => String(c).toLowerCase().trim());
        const t1 = String(type1 || "").toLowerCase().trim();
        const t2 = String(type2 || "").toLowerCase().trim();

        if (t1 && sClasses.some(sc => sc === t1 || sc.includes(t1) || t1.includes(sc))) return true;
        if (t2 && sClasses.some(sc => sc === t2 || sc.includes(t2) || t2.includes(sc))) return true;
        
        if (arr && arr.length > 0) {
            return arr.some(a => {
                const targetA = String(a).toLowerCase().trim();
                return sClasses.some(sc => sc === targetA || sc.includes(targetA) || targetA.includes(sc));
            });
        }
        return false;
      };

      // StudentRecordings එකේ ඇති Payment Fetch කිරීමේ ක්‍රියාවලිය
      ...
      if (calData) {
        ...
        filteredCal.forEach((ev: any) => {
            const standardizedDbMonth = ev.date.substring(0, 7); // yyyy-MM
            const isGloballyFree = currentUser?.plan_type?.toLowerCase() === 'free';
            const isThisMonthFree = currentUser?.free_months?.includes(standardizedDbMonth);
      ...
      if (liveData) {
        ...
        filteredLive.forEach((live: any) => {
            const standardizedDbMonth = live.target_month || live.date.substring(0, 7);
            const isGloballyFree = currentUser?.plan_type?.toLowerCase() === 'free';
            const isThisMonthFree = currentUser?.free_months?.includes(standardizedDbMonth);
      ...
      <div className="flex flex-col items-center min-h-screen bg-black text-white p-4 md:p-8">
        ...
        <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {calendarEvents.map((ev, idx) => {
            const isPaid = paymentStatuses[`cal_${ev.id}`] || false;
            return (
    ...
    <div className="w-full min-h-screen bg-black text-white flex flex-col p-4 md:p-8">
      ...
      {viewState === 'waiting-24h' && targetClass && (
        <div className="flex flex-col items-center justify-center w-full h-[60vh] md:h-[75vh] bg-gray-900 rounded-2xl border border-gray-800 shadow-2xl relative overflow-hidden">
          {renderLockedOverlay()}
          <div className={`z-10 text-center p-6 flex flex-col items-center w-full max-w-2xl ${!isTargetClassPaid ? 'opacity-20 blur-sm pointer-events-none' : ''}`}>
      ...
      {viewState === 'waiting-30m' && targetClass && (
        <div className="flex flex-col items-center justify-center w-full h-[60vh] md:h-[75vh] relative rounded-2xl overflow-hidden bg-gray-900 border border-gray-800 shadow-2xl">
          ...
          <div className={`relative z-10 flex flex-col items-center p-8 bg-black/60 rounded-3xl backdrop-blur-md border border-white/10 shadow-2xl ${!isTargetClassPaid ? 'opacity-20 blur-sm pointer-events-none' : ''}`}>
      ...
      {viewState === 'waiting-0s' && targetClass && (
        <div className="flex flex-col items-center justify-center w-full h-[60vh] md:h-[75vh] relative rounded-2xl overflow-hidden bg-gray-950 border border-green-500/40">
          ...
          <div className={`relative z-10 flex flex-col items-center p-6 bg-black/70 rounded-3xl backdrop-blur-lg border border-green-500/30 text-center shadow-2xl ${!isTargetClassPaid ? 'opacity-20 blur-sm pointer-events-none' : ''}`}>
      ...
      {viewState === 'live' && targetClass && (
        ...
        >
          ...
          <div className={`bg-green-950/90 text-green-400 px-4 py-2 flex items-center justify-between border-b border-green-500/20 text-sm md:text-base z-10 ${!isTargetClassPaid ? 'opacity-20 blur-sm pointer-events-none' : ''}`}>
