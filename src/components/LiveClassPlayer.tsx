...
// Convert Google Drive preview link
...
const LiveClassPlayer = ({ currentUser }: { currentUser: Student }) => {
  ...
  // Realtime Supabase Subscriptions & Data Load
  useEffect(() => {
    initDataFetch();
  ...
  const initDataFetch = async () => {
    ...
    try {
      ...
      // Helper function to match class
      ...
      validLives.forEach((cls: ScheduledLive) => {
        ...
        if (isLive || (diffHours >= -12 && diffHours <= 48)) {
      ...
      // 4. Verify Payments for current class/month
  ...
  const verifyPaymentAccess = async (targetClass: ScheduledLive, student: Student, defaultMonth: string) => {
    ...
    // Check student table free/active flags
    ...
    // Query payments table for this username
  ...
  // Calculate timing parameters for active class
  ...
  // Difference in seconds from current time
  const diffSeconds = activeClass ? Math.floor((classDateTime.getTime() - currentTime.getTime()) / 1000) : 999999;
  
  // Under 12 hours check (43,200 seconds)
  const isWithin12Hours = activeClass && (diffSeconds <= 43200);
  
  // Under 30 minutes check (1,800 seconds)
  const isWithin30Mins = activeClass && (diffSeconds <= 1800);

  // --------------------------------------------------
  // 2. CALENDAR VIEW (If class is more than 12 hours away or no live scheduled)
  // --------------------------------------------------
  if (!isLive && !isWithin12Hours) {
                ...
                return (
                  <div
                    ...
                    className={`min-h-[90px] md:min-h-[120px] p-2 rounded-2xl border transition relative flex flex-col justify-between ${
                      hasEvent
                        ? 'bg-blue-950/30 border-blue-500/60 shadow-[0_0_15px_rgba(59,130,246,0.25)] animate-pulse'
                        : isToday
                  ...
                  >
                    ...
                    {/* Events Info inside Date Box */}
                    {hasEvent && (
                      <div className="mt-2 space-y-1 overflow-hidden">
                        {dayEvents.map(evt => (
                          <div 
                            key={evt.id} 
                            className="bg-blue-600/20 border border-blue-500/40 rounded-lg p-1 text-[10px] text-blue-200 truncate font-semibold"
  ...
  // --------------------------------------------------
  // 3. LAST 12-HOUR LIST VIEW (Calendar disappears, ordered list appears)
  // --------------------------------------------------
  ...
  // --------------------------------------------------
  // 4. LAST 30-MINUTES WAITING VIDEO PLAYER
  // --------------------------------------------------
  if (!isLive && isWithin30Mins) {
  ...
  // --------------------------------------------------
  // 5. LIVE EMBEDDED ZOOM PLAYER + SPLIT SCREEN EXAM
  // --------------------------------------------------
  if (isLive) {
      ...
      <div className="w-full h-screen max-h-screen bg-black text-white flex flex-col overflow-hidden">
        ...
        <div className={`flex-1 w-full ${isExamPushed ? 'flex flex-col lg:flex-row' : 'flex'}`}>
          ...
          <div className={`${isExamPushed ? 'h-[40vh] lg:h-full lg:w-[35%] flex flex-col border-b lg:border-b-0 lg:border-r border-gray-800 bg-gray-900' : 'w-full h-full'}`}>
            ...
            {/* Zoom Frame with Camera & Mic Access */}
            ...
            {/* MCQ OMR Answer Sheet (When Exam Activated) */}
            {isExamPushed && (
          ...
          {/* Right Side: Google Drive PDF Viewer */}
          {isExamPushed && (
